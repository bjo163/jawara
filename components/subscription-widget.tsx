...
export function SubscriptionWidget() {
  ...
  const scrollToContact = () => {
    ...
    setTimeout(() => {
      ...
      if (element) {
        ...
        setTimeout(() => {
          const addressInput = document.querySelector('input[placeholder*="alamat"], textarea[placeholder*="alamat"], input[name*="address"], textarea[name*="address"]') as HTMLInputElement | HTMLTextAreaElement
        ...
        const contactForm = document.querySelector('form, [id*="contact"], [class*="contact"]')
  ...
  const checkCoverage = (userLat: number, userLng: number) => {
    ...
    setTimeout(() => {
      setCoverageStatus(distance <= jawaraNetOffice.coverageRadius ? 'covered' : 'not-covered')
    }, 1500)
  }
              ...
              <div className="text-center">
                ...
                {locationStatus === 'error' && (
                  <div className="text-red-400 text-sm">
                    ...
                    <p>Tidak dapat mengakses lokasi. Pastikan Anda mengizinkan akses lokasi di browser.</p>
                    <Button
                      onClick={getUserLocation}
                        ...
                        <div className="space-y-2">
                          <button
                            onClick={() => {
                              ...
                              // Set lokasi user ke contact form (jika ada)
                              setTimeout(() => {
                                const addressInput = document.querySelector('input[placeholder*="alamat"], textarea[placeholder*="alamat"], input[name*="address"], textarea[name*="address"]') as HTMLInputElement | HTMLTextAreaElement
                                if (addressInput && userLocation) {
                                  addressInput.value = `Koordinat: ${userLocation.lat.toFixed(6)}, ${userLocation.lng.toFixed(6)} (Jarak ${distance.toFixed(1)}km dari kantor)`
