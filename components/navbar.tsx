...
export function Navbar({ activeSection, onNavigate }: NavbarProps) {
  ...
  const navItems = [
    ...
    { id: "contact", label: "Kontak", icon: "📞", type: "link", href: "/contact" },
  ]
        ...
        <div className="flex items-center justify-between h-16">          {/* Logo */}
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleNavigation(item)}                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center space-x-2 ${
          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="relative group">
              ...
              {/* Dropdown Menu */}              <div className="absolute right-0 mt-2 w-48 bg-slate-950 border border-gray-800 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                <div className="py-2">
                  <button
          ...
          <div className="md:hidden absolute top-16 left-0 right-0 bg-slate-950/95 backdrop-blur-md border-b border-gray-800/50 shadow-lg">
            <div className="px-4 py-4 space-y-2">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => handleNavigation(item)}                  className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-colors duration-200 flex items-center space-x-3 ${
                    activeSection === item.id
                      ? "bg-orange-500/20 text-orange-400"
                      : "text-gray-300 hover:text-orange-400 hover:bg-orange-500/10"
                  }`}
              ...
              {/* Mobile Login Buttons */}              <div className="pt-4 border-t border-gray-800">
                <button
                  onClick={() => window.location.href = '/login/pelanggan'}
