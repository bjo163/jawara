  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        ...
        <h3 className={`cartoon-title text-xl font-black ${colors.text} mb-3`}>
          {name}
        </h3>
        <div className="space-y-1">
          <div className="cartoon-text text-3xl font-black text-white">
            {speed}
          </div>
          <div className="cartoon-text text-gray-400 font-semibold">
            {quota}
          </div>
        </div>
      </div>
      ...
      <div className="text-center mb-6">
        <div className="professional-card p-4 bg-gradient-to-br from-slate-800/50 to-slate-700/50">
          <div className="flex items-center justify-center space-x-2 mb-1">
            {originalPrice && (
              <span className="cartoon-text text-gray-500 line-through text-lg">
                {originalPrice}
              </span>
            )}
            <span className="cartoon-text text-2xl font-black text-white">
              {price}
            </span>
          </div>
      ...
      <ul className="space-y-3 mb-8">
        {features.map((feature, index) => (
          <li key={index} className="flex items-start space-x-3 text-sm">
            ...
            <span className="cartoon-text text-gray-300 font-medium">
              {feature}
            </span>
          </li>
        ))}
