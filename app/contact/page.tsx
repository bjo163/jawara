...
export default function ContactPage() {
  ...
  const getUserLocation = () => {
    ...
    if (!navigator.geolocation) {
      setState(prev => ({ ...prev, locationStatus: 'error', coverageStatus: null }))
      return
    }
  ...
  const getColorClasses = (color: string) => {
    ...
    return colorMap[color as keyof typeof colorMap] || 'bg-gray-500/20 text-gray-500'
  }
          ...
          <CardContent>
            {state.locationStatus === 'loading' && (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="cartoon-text text-gray-300">Mendeteksi lokasi Anda...</span>
              </div>
            )}
                ...
                {state.coverageStatus === 'covered' && (
                  <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 p-4 rounded-lg border border-green-500/30">
                    <p className="cartoon-text text-green-300 font-semibold">
                      🎉 Bagus! Lokasi Anda terjangkau layanan kami. Silakan hubungi kami untuk instalasi.
                    </p>
                  </div>
                )}
              </div>
            ...
            {state.locationStatus === 'error' && (
              <div className="bg-yellow-500/20 border border-yellow-500/30 p-4 rounded-lg">
                <div className="flex items-center mb-2">
                  ...
                  <span className="cartoon-text font-bold text-white">Tidak dapat mendeteksi lokasi</span>
                </div>
                <p className="cartoon-text text-gray-300 mb-3">
                  Silakan aktifkan GPS atau berikan izin lokasi untuk pengecekan coverage area.
                </p>
                <Button 
                  ...
                  <div key={contact.id} className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      {contact.isClickable ? (
                        ...
                        <p className="cartoon-text text-gray-300">{contact.value}</p>
                      )}
                      {contact.secondary && (
                        <p className="cartoon-text text-gray-300">{contact.secondary}</p>
                      )}
                      <p className="cartoon-text text-gray-400 text-sm">{contact.description}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {quickActions.map((action) => (
                    <a
                      key={action.id}
                      href={action.href}
                      target={action.id === 'whatsapp-action' ? '_blank' : undefined}
                      rel={action.id === 'whatsapp-action' ? 'noopener noreferrer' : undefined}
